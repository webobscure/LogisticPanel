...
export default function RacesToday() {
  ...
  const statuses = [
    ...
    { value: "Назначен водитель", label: "Назначен водитель" },
  ...
  const saveTrip = async () => {
    try {
      ...
      if (selectedTrip.driverId)
        params.append("user_id", selectedTrip.driverId);
      if (selectedTrip.status) params.append("status", selectedTrip.status);
      if (selectedTrip.loadingTime)
        params.append("loading_time", selectedTrip.loadingTime);
      if (selectedTrip.loadingCountry)
        params.append("loading_country", selectedTrip.loadingCountry);
      if (selectedTrip.loadingCity)
        params.append("loading_city", selectedTrip.loadingCity);
      if (selectedTrip.loadingStreet)
        params.append("loading_street", selectedTrip.loadingStreet);
      if (selectedTrip.unloadingTime)
        params.append("unloading_time", selectedTrip.unloadingTime);
      if (selectedTrip.unloadingCountry)
        params.append("unloading_country", selectedTrip.unloadingCountry);
      if (selectedTrip.unloadingCity)
        params.append("unloading_city", selectedTrip.unloadingCity);
      if (selectedTrip.unloadingStreet)
        params.append("unloading_street", selectedTrip.unloadingStreet);
      if (selectedTrip.customerName)
        params.append("customer_name", selectedTrip.customerName);
      if (selectedTrip.customerContacts)
        params.append("customer_contacts", selectedTrip.customerContacts);
      if (selectedTrip.weight)
        params.append("cargo_weight", selectedTrip.weight);
      if (selectedTrip.height)
        params.append("cargo_height", selectedTrip.height);
      if (selectedTrip.width) params.append("cargo_width", selectedTrip.width);
      if (selectedTrip.length)
        params.append("cargo_length", selectedTrip.length);
      ...
      const res = await fetch(
        `${API_URL}/logist-order?id=${selectedTrip.id}&${params.toString()}`,
        {
          method: "PATCH",
          headers: {
  ...
  const createTrip = async () => {
    try {
      if (
        !newTrip.customerName ||
        !newTrip.customerContacts ||
        !newTrip.price
      ) {
        alert("Пожалуйста, заполните имя клиента, контакты и цену.");
        return;
  ...
  const cancelTrip = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/logist-order?id=${selectedTrip.id}&status=Отменен`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` },
        }
          ...
          {["weight", "height", "width", "length"].map((f) => (
            <div key={f} className="details-item">
              <label>
                {f === "weight"
                  ? "Вес (кг)"
                  : f.charAt(0).toUpperCase() + f.slice(1) + " (мм)"}
              </label>
              <input
                type="number"
                className="ui-input"
        ...
        <div className="details-grid">
          ...
          <div className="details-item" style={{ gridColumn: "span 2" }}>
            ...
            <input
              ...
              value={trip.price || ""}
              onChange={(e) =>
                handleFieldChange("price", e.target.value, isNew)
              }
            />
          </div>
        </div>
  ...
  return (
    <div className="racestoday bg-card-light">
      ...
      {selectedTrip && (
        <UiModal title={`Рейс #${selectedTrip.id}`} onClose={closeTripModal}>
          {renderTripForm(selectedTrip)}
        </UiModal>
      )}

      {newTripModal && (
        <UiModal title="Добавить новый рейс" onClose={closeNewTripModal}>
